...
# Semivowel mapping dictionary for basic vowels
SEMIVOWEL_MAPPING = {
    'ㅏ': ['ㅑ', 'ㅘ'],
    'ㅓ': ['ㅕ', 'ㅝ'],
    'ㅗ': ['ㅛ'],
...
def phonological_addition_semivowel(text: str, ratio: float = 0.3) -> str:
    """
    ...
    def chunk_process(chunk: str) -> Optional[str]:
        ...
        for char in chunk:
            ...
            if hgtk.checker.is_hangul(char):
                ...
                if jung in SEMIVOWEL_MAPPING:
                    # Apply semivowel transformation
                    new_jung = random.choice(SEMIVOWEL_MAPPING[jung])
...
def phonological_addition_adaptive_final_consonant(text: str, ratio: float = 0.3) -> str:
    """
            ...
            if hgtk.checker.is_hangul(char):
                ...
                if jong == '' and i + 1 < len(chars):
                    ...
                    if hgtk.checker.is_hangul(next_char):
                        ...
                        if next_cho in INITIAL_TO_FINAL_MAPPING:
                            new_jong = INITIAL_TO_FINAL_MAPPING[next_cho]
...
def phonological_addition_initial_consonant(text: str, ratio: float = 0.3) -> str:
    """
            ...
            if hgtk.checker.is_hangul(char):
                ...
                if cho == 'ㅇ' and i > 0:
                    ...
                    if hgtk.checker.is_hangul(prev_char):
                        ...
                        if prev_jong != '' and prev_jong in FINAL_TO_INITIAL_MAPPING:
                            new_cho = FINAL_TO_INITIAL_MAPPING[prev_jong]
...
def phonological_addition_final_consonant(text: str, ratio: float = 0.3, double_consonant_ratio: float = 0.2) -> str:
    """
            ...
            if hgtk.checker.is_hangul(char):
                ...
                if jong == '':
                    ...
                    if random.random() < double_consonant_ratio:
                        new_jong = random.choice(DOUBLE_FINAL_CONSONANTS)
                    else:
                        new_jong = random.choice(SINGLE_FINAL_CONSONANTS)
...
def korean_obscure(text: str, semivowel: bool = False, initial_consonant: bool = False, 
    ...
    if semivowel:
        result = phonological_addition_semivowel(result)
    if initial_consonant:
        result = phonological_addition_initial_consonant(result)
    if final_consonant:
        result = phonological_addition_final_consonant(result, double_consonant_ratio=double_consonant_ratio)
    if adaptive_final_consonant:
        result = phonological_addition_adaptive_final_consonant(result)
...
if __name__ == "__main__":
    ...
    print(korean_obscure(test_text, semivowel=True))
    
    print("\n=== Initial Consonant Addition Test ===")
    print(korean_obscure(test_text, initial_consonant=True))
    
    print("\n=== Random Final Consonant Addition Test ===")
    print(korean_obscure(test_text, final_consonant=True))
    
    print("\n=== Adaptive Final Consonant Addition Test ===")
    print(korean_obscure(test_text, adaptive_final_consonant=True))
    
    print("\n=== Combined Test (All Features) ===")
    print(korean_obscure(test_text, semivowel=True, initial_consonant=True, adaptive_final_consonant=True))
    ...
    for word in initial_test_words:
        initial_result = phonological_addition_initial_consonant(word, ratio=1.0)
    ...
    for word in adaptive_test_words:
        adaptive_result = phonological_addition_adaptive_final_consonant(word, ratio=1.0)
    ...
    for word in test_words:
        semivowel_result = phonological_addition_semivowel(word, ratio=1.0)
        initial_result = phonological_addition_initial_consonant(word, ratio=1.0)
        adaptive_final_result = phonological_addition_adaptive_final_consonant(word, ratio=1.0)
        print(f"{word}")
    ...
    for word in complex_examples:
        result = korean_obscure(word, semivowel=True, initial_consonant=True, adaptive_final_consonant=True)
        print(f"{word} -> {result}")
